...
def scan(root: Path):
    findings = []
    files = [p for p in root.rglob("*") if p.is_file() and p.suffix.lower() in TEXT_EXT and p.name != "__pycache__"]
    for p in files:
        try: text = p.read_text(encoding="utf-8", errors="replace")
        except OSError as e:
            findings.append(("HIGH", str(p), f"unreadable: {e}")); continue
        rel = str(p.relative_to(root))
        if rel == "runtime/incoming-skill-gate.py": continue
        scan_text = GITHUB_EXPR.sub("${{ <github-expression> }}", text)
        if SECRET.search(scan_text): findings.append(("CRITICAL", rel, "possible hard-coded secret"))
        if INJECTION.search(scan_text): findings.append(("HIGH", rel, "instruction override / concealment pattern"))
...
def main():
    ...
    if not args.path.exists() or not args.path.is_dir(): ap.error("path must be an existing directory")
    findings, files = scan(args.path); digest = hashlib.sha256()
